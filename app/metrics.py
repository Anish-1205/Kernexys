...
class MetricsCollector:
    ...
    def to_prometheus_format(self) -> str:
        ...
        for (method, path, status), count in self._request_count.items():
            lines.append(
                f'kernexys_http_requests_total{{method="{method}",path="{path}",status="{status}"}} {count}'
            )
        ...
        for (method, path, status), durations in self._request_latency.items():
            if durations:
                ...
                lines.append(
                    f'kernexys_http_request_duration_seconds_p50{{method="{method}",path="{path}",status="{status}"}} {p50}'
                )
                lines.append(
                    f'kernexys_http_request_duration_seconds_p95{{method="{method}",path="{path}",status="{status}"}} {p95}'
                )
                lines.append(
                    f'kernexys_http_request_duration_seconds_p99{{method="{method}",path="{path}",status="{status}"}} {p99}'
                )
        ...
        for (deployment, error_type), count in self._queue_errors.items():
            lines.append(
                f'kernexys_async_queue_errors_total{{deployment="{deployment}",type="{error_type}"}} {count}'
            )
        ...
        for deployment, syncs in self._kubernetes_syncs.items():
            if syncs:
                ...
                if successful:
                    ...
                    p99 = sorted(durations)[int(len(durations) * 0.99)]
                    lines.append(
                        f'kernexys_kubernetes_sync_duration_seconds_p50{{deployment="{deployment}"}} {p50}'
                    )
                    lines.append(
                        f'kernexys_kubernetes_sync_duration_seconds_p95{{deployment="{deployment}"}} {p95}'
                    )
                    lines.append(
                        f'kernexys_kubernetes_sync_duration_seconds_p99{{deployment="{deployment}"}} {p99}'
                    )
        ...
        for deployment, count in self._kubernetes_sync_errors.items():
            lines.append(f'kernexys_kubernetes_sync_errors_total{{deployment="{deployment}"}} {count}')
